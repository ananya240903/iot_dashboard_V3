...
import { OFFICE_STATUS_COLORS } from '../../utils/theme';
...
export default function DeviceTypeStatus() {
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col justify-center items-center h-[500px] bg-white/90 backdrop-blur-2xl rounded-3xl shadow-sm border border-slate-200/60 mb-8 transition-all duration-500">
                <RefreshCw className="animate-spin text-[var(--color-office-blue)] mb-4" size={36} strokeWidth={2.5} />
    ...
    return (
        <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
            {/* Header */}
            <div className="bg-[rgba(255,255,255,0.9)] backdrop-blur-3xl rounded-[2rem] p-8 shadow-[0_12px_30px_rgba(56,93,138,0.08)] border border-[rgba(121,151,188,0.16)] relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-96 h-96 bg-[rgba(79,129,189,0.12)] rounded-full blur-3xl -translate-y-1/2 translate-x-1/3 group-hover:scale-110 transition-transform duration-700"></div>
                <div className="absolute bottom-0 left-0 w-64 h-64 bg-[rgba(247,150,70,0.12)] rounded-full blur-3xl translate-y-1/2 -translate-x-1/2 group-hover:scale-110 transition-transform duration-700"></div>

                <div className="relative z-10 flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-extrabold text-transparent bg-clip-text bg-linear-to-r from-slate-900 via-[var(--color-office-blue-dark)] to-[var(--color-office-purple)] flex items-center gap-3">
                            <div className="p-3 bg-[rgba(220,230,242,0.9)] rounded-2xl">
                                <Activity className="text-[var(--color-office-blue)]" size={28} strokeWidth={2.5} />
                            </div>
                            Device Type Status
                        </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {typeStats.map((stat, idx) => {
                    ...
                    return (
                        <div key={idx} className="bg-[rgba(255,255,255,0.86)] backdrop-blur-xl rounded-[2rem] p-6 shadow-[0_12px_28px_rgba(56,93,138,0.08)] border border-[rgba(121,151,188,0.14)] hover:-translate-y-1 transition-all duration-300 group">
                            <div className="flex items-center justify-between mb-2">
                                <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                                    <PieChartIcon className="text-[var(--color-office-blue)]" size={20} />
                                    {stat.name}
                                </h3>
                                <span className={`text-xs font-bold px-3 py-1 rounded-full ${livePercentage >= 80 ? 'bg-[rgba(234,241,221,0.95)] text-[#76923C]' : 'bg-[rgba(242,220,219,0.95)] text-[#953735]'}`}>
                                    {livePercentage}% Live
                                </span>
                            </div>
                            ...
                            <div className="h-64 w-full relative">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                    ...
                                    <span className="text-[10px] font-bold text-[var(--color-office-green)] tracking-wider uppercase">Live</span>
                                </div>
                            </div>

                            <div className="flex items-center justify-center gap-6 mt-4">
                                <div className="flex items-center gap-2">
                                    <div className="w-3 h-3 rounded-full bg-[var(--color-office-green)]"></div>
                                    <span className="text-sm font-semibold text-slate-600">Live: {stat.live}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-3 h-3 rounded-full bg-[var(--color-office-red)]"></div>
                            ...
                            <div className="mt-6 border border-slate-200/60 rounded-xl overflow-hidden transition-all duration-300 bg-white">
                                ...
                                {revealedCards[stat.name] && (
                                    <div className="p-4 bg-white border-t border-slate-200/60 flex flex-col gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
                                        <div 
                                            className={`flex items-center justify-between p-2 rounded-lg transition-colors ${stat.alerts > 0 ? 'bg-[rgba(253,234,218,0.86)] cursor-pointer hover:bg-[rgba(253,234,218,1)]' : 'bg-slate-50'}`}
                                            onClick={() => {
                                                if (stat.alerts > 0) {
                                        ...
                                        >
                                            ...
                                            <span className={`text-sm font-bold ${stat.alerts > 0 ? 'text-[var(--color-office-orange)]' : 'text-slate-500'}`}>{stat.alerts}</span>
                                        </div>
                                        <div 
                                        ...
                                        <div 
                                            className={`flex items-center justify-between p-2 rounded-lg border transition-colors ${stat.critical > 0 ? 'bg-[rgba(242,220,219,0.8)] border-[rgba(192,80,77,0.2)] cursor-pointer hover:bg-[rgba(242,220,219,1)]' : 'bg-slate-50 border-slate-100'}`}
                                            onClick={() => {
                                                if (stat.critical > 0) {
                                        ...
                                        >
                                            <span className={`text-xs font-bold uppercase ${stat.critical > 0 ? 'text-[#953735]' : 'text-slate-400'}`}>Critical</span>
                                            <span className={`text-sm font-bold ${stat.critical > 0 ? 'text-[var(--color-office-red)]' : 'text-slate-400'}`}>{stat.critical}</span>
                                        </div>
                                        {stat.others > 0 && (
                                            <div 
                                                className={`flex items-center justify-between p-2 rounded-lg border transition-colors bg-[rgba(218,238,243,0.8)] border-[rgba(75,172,198,0.2)] cursor-pointer hover:bg-[rgba(218,238,243,1)]`}
                                                onClick={() => {
                                            ...
                                            >
                                                <span className="text-xs font-bold uppercase text-[#31859B]">Other Severities</span>
                                                <span className="text-sm font-bold text-[var(--color-office-teal)]">{stat.others}</span>
                                            </div>
                                        )}
                                    </div>
                                )}
