...
export default function Sidebar() {
  ...
  return (
    <aside
      className={`${isOpen ? 'w-72' : 'w-[80px]'} bg-white  border-r border-slate-200/50  flex flex-col z-10 transition-all duration-300 ease-in-out shrink-0 relative shadow-sm`}
    >
      {/* Toggle Button */}
      <div className="absolute -right-3 top-6 z-20">
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="bg-white border border-slate-200 shadow-sm text-slate-400 hover:text-indigo-600 rounded-full p-1.5 transition-all hover:scale-110 focus:outline-none"
      ...
      <div className="p-4 flex-1 flex flex-col overflow-y-auto custom-scrollbar">
        {navItems.map((group, gi) => (
          <div key={gi}>
            {isOpen && (
              <h3 className={`px-4 text-[11px] font-extrabold text-slate-400 uppercase tracking-widest mb-4 ${gi === 0 ? 'mt-2' : 'mt-8'}`}>
                {group.section}
              </h3>
            )}
              ...
              {group.items.map(({ to, icon: Icon, label }) => (
                <NavLink
                  ...
                  className={() => {
                    ...
                    return `group relative flex items-center ${isOpen ? 'px-4' : 'justify-center'} w-full py-3 rounded-r-xl transition-all duration-300 ${
                      isActive
                        ? 'bg-gradient-to-r from-indigo-500/15 to-transparent text-indigo-700  font-bold'
                ...
                >
                  {() => {
                    ...
                    <>
                      {isActive && (
                        <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-8 bg-indigo-600 rounded-r-full shadow-[0_0_8px_rgba(79,70,229,0.6)]"></div>
                      )}
                      <div className={`flex items-center ${isOpen ? 'gap-3 w-full' : 'justify-center w-full'}`}>
                        <Icon
                          ...
                          className={`${
                            isActive
                              ? 'text-indigo-600  drop-shadow-md'
