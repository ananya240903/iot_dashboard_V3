...
import { Search, ArrowUpDown, ArrowUp, ArrowDown, ArrowLeft, RefreshCw } from 'lucide-react';
import { useSearchParams, useNavigate } from 'react-router-dom';
import { useDeviceStore } from '../../store/useDeviceStore';
import { useVendorStore } from '../../store/useVendorStore';
import calculate from '../../components/DeviceData/calculate';

export default function ZoneConnectionDetails() {
    const [searchParams] = useSearchParams();
    const navigate = useNavigate();
    const connectionFilter = searchParams.get('filter') || 'all';
    ...
    const connectionStats = useMemo(() => {
        return calculate(data, { zone: filterZone, deviceType: ["All"], status: "All" }).connectionStats;
    }, [data, filterZone]);

    const filteredConnectionDetails = useMemo(() => {
        if (!connectionStats?.details) return [];
        let details = connectionFilter === 'all' ? connectionStats.details : connectionStats.details.filter(d => d.status === connectionFilter);
        if (filterVendor !== 'All') {
        ...
        return details;
    }, [connectionFilter, connectionStats, searchQuery, filterVendor]);


    useEffect(() => {
        setCurrentPage(1);
    }, [connectionFilter, searchQuery, filterZone, filterVendor]);

    const totalPages = Math.ceil(filteredConnectionDetails.length / itemsPerPage);
    const paginatedDetails = filteredConnectionDetails.slice(
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-extrabold text-slate-800 capitalize tracking-tight">
                            {connectionFilter === 'all' ? 'All Connection Details' : `${connectionFilter} Devices`}
                            {filterZone !== 'All' && ` (${filterZone})`}
                        </h2>
                        <p className="text-slate-500 font-medium text-sm mt-1">
                            Showing {filteredConnectionDetails.length} device(s)
                        </p>
                    </div>
                </div>
            ...
            <div className="flex-1 overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
                <table className="min-w-full divide-y divide-slate-100">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {paginatedDetails.map((detail, idx) => (
                            ...
                            >
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                    <span className={`px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full border ${detail.status === 'live' ? 'bg-emerald-50 text-emerald-600 border-emerald-200   ' : 'bg-rose-50 text-rose-600 border-rose-200   '
                                        }`}>
                                        {detail.status}
                                    </span>
                                </td>
                        ...
                        {filteredConnectionDetails.length === 0 && (
                            <tr>
